...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (primaryResult.ok) {
      ...
      const finalizedData = finalizeResultData(mergedData);
      if (sampleCacheKey) {
    ...
    if (fallbackResult.ok) {
      const finalizedData = finalizeResultData(fallbackResult.data);
      if (sampleCacheKey) {
        writeSampleResultCache(sampleCacheKey, finalizedData, SAMPLE_RESULT_CACHE_TTL_MS);
      }
      return res.status(200).json({ ok: true, data: finalizedData });
    ...
    if (platform === "spotify") {
      ...
      if (spotifyFallback.ok) {
        const finalizedData = finalizeResultData(spotifyFallback.data);
        if (sampleCacheKey) {
          writeSampleResultCache(sampleCacheKey, finalizedData, SAMPLE_RESULT_CACHE_TTL_MS);
        }
        return res.status(200).json({ ok: true, data: finalizedData });
...
function finalizeResultData(data) {
  const payload = data || {};
  ...
  return {
    ...
    description: sanitizeMetadataText(payload.description, MAX_METADATA_TEXT_LENGTH),
...
function sanitizeMetadataText(value, maxLen = MAX_METADATA_TEXT_LENGTH) {
