...
async function buildSpotifySearchFallback(link) {
  ...
  try {
    ...
    const successResult = {
      ...
      status: 200,
      data: {
        title: metadata.title || appleMusicResult.title || "música encontrada",
        description: metadata.description || normalizedQuery.artist || appleMusicResult.artist || "",
        album: "",
        image: metadata.image || appleMusicResult.image || "",
        universalLink: "",
        links
      }
    };
...
async function buildSearchFallbackFromQuery(query) {
  ...
  const cached = readCache(queryModeCache, queryKey);
  ...
  const songLinkFromApple = appleMusicResult.url
    ? await withTimeout(fetchSongLink(appleMusicResult.url, { markVerified: true }), 2200, { ok: false })
    : { ok: false };
  ...
  const result = {
    ...
    status: 200,
    data: {
      title: appleMusicResult.title || normalizedQuery.title || query,
      description: appleMusicResult.artist || normalizedQuery.artist || "",
      album: "",
      image: appleMusicResult.image || "",
      universalLink: "",
      links
    }
  };
  writeCache(queryModeCache, queryKey, result, QUERY_MODE_CACHE_TTL_MS);
  return result;
}
...
async function fetchAppleMusicLinkFromItunes(query, normalizedQuery) {
  if (!query) return { url: "", isVerified: false, title: "", artist: "", image: "" };
  ...
  try {
    ...
    if (!response.ok) return { url: "", isVerified: false, title: "", artist: "", image: "" };

    const data = await response.json();
    const results = Array.isArray(data?.results) ? data.results : [];
    if (!results.length) return { url: "", isVerified: false, title: "", artist: "", image: "" };
    ...
    return {
      ...
      image: target?.candidate?.artworkUrl100
        ? String(target.candidate.artworkUrl100).replace(/100x100bb/i, "600x600bb")
        : ""
    };
  } catch (_error) {
    return { url: "", isVerified: false, title: "", artist: "", image: "" };
  }
}
