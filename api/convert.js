...
async function buildSpotifyInputResolution(link) {
  ...
  const canonicalTitleSelection = selectCanonicalSpotifyInputField([
    ...
    { source: "songlink", value: bridgeResult?.data?.title },
    { source: "itunes_lookup", value: appleMusicResult?.title }
  ]);
  const canonicalArtistSelection = selectCanonicalSpotifyInputField([
    ...
    { source: "songlink", value: songLinkArtist },
    { source: "itunes_lookup", value: appleMusicResult?.artist }
  ]);
  const canonicalAlbumSelection = selectCanonicalSpotifyInputField([
    ...
    { source: "songlink", value: songLinkAlbum },
    { source: "itunes_lookup", value: appleMusicResult?.album }
  ]);

  const metadataPayload = pickBestMetadata(
    {
      title: canonicalTitleSelection.value || "música encontrada",
      description: canonicalArtistSelection.value || "",
      album: canonicalAlbumSelection.value || "",
      image: metadata?.image || ""
    },
  ...
  console.log(
    JSON.stringify({
      ...
      title: { value: metadataPayload.title, source: canonicalTitleSelection.source || "primary_api" },
      artist: { value: metadataPayload.description, source: canonicalArtistSelection.source || "primary_api" },
      album: { value: metadataPayload.album, source: canonicalAlbumSelection.source || "primary_api" },
      trackUrl: spotifyUrl
    })
  );
  ...
  return {
    ...
    data: {
      ...metadataPayload,
      _lockArtist: true,
      _spotifyCanonicalMetadata: {
        title: canonicalTitleSelection.value || "",
        artist: canonicalArtistSelection.value || "",
        album: canonicalAlbumSelection.value || "",
        titleSource: canonicalTitleSelection.source || "",
        artistSource: canonicalArtistSelection.source || "",
        albumSource: canonicalAlbumSelection.source || ""
      },
      links: mergedLinks
    }
  };
...
async function finalizeResultData(data) {
  ...
  const { _lockArtist, _spotifyCanonicalMetadata, ...publicPayload } = canonicalApplied || {};
  return publicPayload;
}

function applySpotifyCanonicalMetadata(payload) {
  const canonical = payload?._spotifyCanonicalMetadata || {};
  if (!canonical || (!canonical.title && !canonical.artist && !canonical.album)) return payload;

  return {
    ...payload,
    title: canonical.title || payload.title || "",
    description: canonical.artist || payload.description || "",
    album: canonical.album || payload.album || ""
  };
}
